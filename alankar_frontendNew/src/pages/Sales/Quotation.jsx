...
const QuotationPopup = ({
  ...
  const handleFormSubmit = async (e) => {
    ...
    const result = await handleAddQuotation(presale.srNumber, {
      details: form.details,
      totalAmount: Number(form.totalAmount),
      // "sended" and "isAccepted" can be default false
      sended: false,
      isAccepted: false,
    });
    if (result.success) {
  ...
  return (
    <div className={styles.modalBackdrop} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className={styles.modalBox} style={{ maxWidth: 850 }}>
        <div className={styles.modalHeader}>
          <span>
            Quotations for: <b>{presale?.personName}</b> ({presale?.client?.clientName})<br />
            <small style={{ fontWeight: 400 }}>Requirements: {presale?.requirements}</small>
        ...
        <div className={styles.modalBody}>
          ...
          {!showForm ? (
            ...
            <form className={styles.modalBody} onSubmit={handleFormSubmit}>
              <div className={styles.inputRow}>
                <label>Details</label>
                <input className={styles.detailsInput}
                  name="details"
                  value={form.details}
                  readOnly // make details read-only
                />
              </div>
              <div className={styles.inputRow}>
                <label>Total Amount</label>
