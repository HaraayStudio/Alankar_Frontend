...
function getPriceDetails({ requirements, clientType = "Cash", orderType = "Wide format printing" }) {
  // Treat GST as Cash if rates are same (or change to your logic)
  if (clientType === "GST") clientType = "Cash";
  const items = parseRequirements(requirements);
  const printTypesObj = PRINT_PRICES.clientTypes?.[clientType]?.orderTypes?.[orderType]?.printTypes || {};
  ...
  items.forEach(item => {
    if (
      !detectedPrintType &&
      Object.keys(printTypesObj).some(pt => pt.toLowerCase() === item.toLowerCase())
    ) {
  ...
  const categories = ["Media", "Lamination", "Mounting", "Installation", "Framing"];
  ...
  return (
    ...
    >
      <div className={styles.previewBox}>
        ...
        <table className={styles.detailsTable}>
          ...
          <tfoot>
            ...
            <tr>
              ...
              <td style={{ fontWeight: "bold", color: "#f4a835" }}>
        ...
        <div className={styles.termsTitle}>TERMS & CONDITIONS:</div>
        <div className={styles.termsBody}>
          Total amount appearing on this invoice shall be paid within 10 days from
          the receipt therefrom.
        </div>
        <div className={styles.footer}>
          <div>Address: {client.address || "--"}</div>
          <div>
