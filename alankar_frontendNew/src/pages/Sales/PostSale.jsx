...
//                 <th style={styles.th}>cleint Name</th>
...
//                  { console.log(item.client.map)}
//                   <td style={styles.td}>{String(index + 1).padStart(2, '0')}</td>
//                   <td style={styles.td}>{item.client?.clientName || '-'}</td>
...
//                   </td>
//                   <td style={styles.td}>{item.conclusion || 'NA'}</td>
//                   <td style={styles.td}>
...
import React, { useState, useEffect } from 'react';
import { Eye, Edit, Trash2, Plus, X, Send } from 'lucide-react';
import { useData } from '../../context/DataContext';
import styles from './PostSalePage.module.scss';
const initialForm = {
  srNumber: '',
...
const PostSale = () => {
  const {
    ...
    handleGetAllPostSales, handleCreatePostSale, handleUpdatePostSale, handleSendPostSaleMail
  } = useData();
  const [showModal, setShowModal] = useState(false);
  ...
  useEffect(() => { handleGetAllPostSales(); }, []);
  const openCreate = () => {
    setForm(initialForm);
  ...
  const handleChange = (e) => {
    ...
    if (dataset.type === 'client') {
      setForm((f) => ({
        ...f,
        client: { ...f.client, [name]: value }
      }));
    } else if (dataset.type === 'order') {
      setForm((f) => ({
        ...f,
        order: { ...f.order, [name]: value }
      }));
    } else {
      setForm((f) => ({
        ...f,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Prepare structure as per backend
    const data = {
      srNumber: form.srNumber,
      client: { clientId: form.client.clientId, clientName: form.client.clientName },
      order: {
        id: form.order.id,
        description: form.order.description,
        createdAt: form.order.createdAt,
        startDate: form.order.startDate,
        endDate: form.order.endDate,
        status: form.order.status,
        priority: form.order.priority,
        type: form.order.type,
      }
    };
    if (isEdit) {
      await handleUpdatePostSale(data);
    } else {
      await handleCreatePostSale(data, isOldClient);
    }
    setShowModal(false);
    setForm(initialForm);
  };
  const handleMail = async (srNumber) => {
    setMailing(srNumber);
    await handleSendPostSaleMail(srNumber);
    setMailing(null);
  };
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>Post-Sales</h1>
        <button className={styles.addButton} onClick={openCreate}>
          <Plus size={18} /> Add PostSales
        </button>
      </div>
          ...
          <table className={styles.table}>
            <thead className={styles.tableHeader}>
              <tr>
                ...
                <th className={styles.th}>Order Description</th>
            ...
            <tbody>
              {postSales?.length === 0 && (
                <tr><td className={styles.td} colSpan={6}>No data</td></tr>
              )}
              {postSales?.map((item, i) => (
                <tr key={item.srNumber}>
                  <td className={styles.td}>{item.srNumber}</td>
                  <td className={styles.td}>{item.client?.clientName || '-'}</td>
                  <td className={styles.td}>{item.order?.description || '-'}</td>
                  ...
                  <td className={styles.td}>
                    <div className={styles.actionButtons}>
                      ...
                      <button className={styles.actionButton} onClick={() => openEdit(item)}><Edit size={14} /></button>
                      {/* <button className={`${styles.actionButton} ${styles.deleteButton}`} onClick={() => handleDeleteClick(item.srNumber)}><Trash2 size={14} /></button> */}
                    </div>
                  </td>
                  <td className={styles.td}>
                    <button
                      className={styles.mailButton}
                      disabled={mailing === item.srNumber}
                      onClick={() => handleMail(item.srNumber)}
                    >
                      <Send size={14} />
                      {mailing === item.srNumber ? "Sending..." : "Send Mail"}
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <div className={styles.modalBackdrop} onClick={(e) => e.target === e.currentTarget && setShowModal(false)}>
          <div className={styles.modalBox}>
            <div className={styles.modalHeader}>
              <h2>{isEdit ? 'Update' : 'Add'} Post Sale</h2>
            ...
            <form className={styles.modalBody} onSubmit={handleSubmit}>
              ...
              <input
                ...
                data-type="order"
                type="datetime-local"
                value={form.order.createdAt ? form.order.createdAt.slice(0, 16) : ''}
                onChange={handleChange}
                className={styles.input}
              />
              ...
              <input
                ...
                data-type="order"
                type="datetime-local"
                value={form.order.startDate ? form.order.startDate.slice(0, 16) : ''}
                onChange={handleChange}
                className={styles.input}
              />
              ...
              <input
                ...
                data-type="order"
                type="datetime-local"
                value={form.order.endDate ? form.order.endDate.slice(0, 16) : ''}
                onChange={handleChange}
                className={styles.input}
              />
              <label>Status</label>
              ...
              <input
                name="type"
                data-type="order"
                value={form.order.type}
                onChange={handleChange}
                className={styles.input}
                placeholder="Type"
              />
              {/* isOldClient checkbox for create */}
              {!isEdit && (
              ...
              <button className={styles.submitButton} type="submit">{isEdit ? 'Update' : 'Create'}</button>
            </form>
          </div>
        </div>
      )}
